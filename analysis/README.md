...
### Third explicit-content pass (2026-09-22)

A further targeted re-scan, prompted by noticing a live example while reviewing `plans/video_streaming.json`, found 26 more rows the first two passes had missed — several were explicit domains in plain English that should have been caught the first time, not just an uncovered-language gap. All 26 removed from `current_origins_verified.csv`; 7 of those had already been merged into `plans/video_streaming.json` and 19 into `plans/web_browsing.json`, removed from both. Two near-miss false positives were checked and correctly kept: a legitimate Russian pop-science site whose domain name coincidentally uses a common English word as branding, and a Brazilian public-school portal whose domain abbreviation happened to collide with an unrelated filter term. File went from 12,303 → 12,277 rows. Given three passes have each independently found gaps the previous ones missed, this filter should still be treated as best-effort, not exhaustive.

### Fourth pass — targeted manual catches (2026-09-23)

Three more explicit-content domains were caught individually while reviewing live collector output and plan files, rather than through a broad keyword re-scan: one whose own fetched description named its real audience plainly once actually read, and two recognized from real-world knowledge of what those domains/brand names are known for, despite each having a bland, generic fetched description that a keyword filter alone would not catch. This is the recurring pattern across all four passes so far — fetched title/description text is not a reliable enough signal on its own, since site operators routinely describe explicit content in vague, filter-evading terms. All three removed from `current_origins_verified.csv` and their corresponding plan files. File went from 12,277 → 12,274 rows.
